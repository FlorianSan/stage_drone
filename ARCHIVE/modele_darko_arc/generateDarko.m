...
A_f = (1-(Swet/(4*Sp))*PHI_fv(1,1));
B_f = -(Swet/(4*Sp))*PHI_fv(3,3)*Thetaf(2,1);
C_f = (Swet/(4*Sp))*Cy0*Thetaf(2,1);
%B = 0;

drone.Fb = [A_f A_f 0 0; 0 0 C_f C_f; 0 0 B_f B_f];

% Calcul du moment
A_m = (km/kf);
B_m = drone.P_A1_CG(2,1)*PHI_fv(3,3)*Thetam(2,1);
C_m = (Swet/(4*Sp))*dR*PHI_fv(3,3)*Thetam(2,1);
D_m = (drone.P_P1_CG(2,1) + (Swet/(4*Sp))*drone.P_A1_CG(1,1)*PHI_fv(1,1));
E_m = (Swet/(4*Sp)) * drone.WINGSPAN/drone.CHORD * dR * Cy0;

drone.Mb = [A_m -A_m  B_m -B_m ; 0 0 C_m C_m ; D_m -D_m E_m E_m ];
